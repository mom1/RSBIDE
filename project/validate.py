"""
    ...
    Either returns False, if no completions are possible or a dictionary containing required paths
    and folders of the project, file and completion base directory. These folders are required to
    build the required path completions.
"""
import re
import os
import sublime
...
def view(view, config, open_dialog=False):
    ...
    config["BASE_DIRECTORY"] = sanitize_base_directory(
        config["BASE_DIRECTORY"], directory["project"], directory["base"])
